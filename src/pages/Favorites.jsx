...
//     setFiltered(result);
//   }, [moodFilter, timeFilter, search, recipes, lang]);
...
//             <div
//               key={recipe._id}
//               className="relative bg-white border rounded-2xl shadow-md hover:shadow-xl overflow-hidden transition-transform duration-200 hover:scale-105"
//             >
...
//     if (search.trim() !== "") {
//       result = result.filter((r) =>
//         r.title?.en?.toLowerCase().includes(search.toLowerCase())
//       );
//     }
//     setFiltered(result);
...
//   return (
//     <div className="max-w-6xl mx-auto px-4 py-10">
//       <h1 className="text-3xl font-bold text-[#2d4032] mb-6 text-center">{t("title")}</h1>
...
//         />
//         <div className="flex gap-4">
//           <select value={moodFilter} onChange={(e) => setMoodFilter(e.target.value)} className="px-3 py-2 border rounded-lg shadow">
//             {Array.isArray(moodOptions) && moodOptions.map(({ key, label }) => (
//               <option key={key} value={key}>{label}</option>
//             ))}
//           </select>
//           <select value={timeFilter} onChange={(e) => setTimeFilter(e.target.value)} className="px-3 py-2 border rounded-lg shadow">
//             {Array.isArray(timeOptions) && timeOptions.map(({ key, label }) => (
//               <option key={key} value={key}>{label}</option>
//             ))}
//           </select>
//         </div>
...
//               <button
//                 onClick={(e) => {
//                   e.stopPropagation(); // ❗ منع الضغط على البطاقة
//                   removeFromFavorites(recipe._id);
//                 }}
//                 className="absolute top-2 left-2 bg-white rounded-full shadow p-1"
//               >
//                 ❌
//               </button>
//             </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((recipe) => (
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation(); // ما يخليش البطاقة تفتح
                  removeFromFavorites(recipe._id);
                }}
